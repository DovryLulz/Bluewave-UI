...
minimizeButton.MouseButton1Click:Connect(function()
    if minimized then
        CreateTween(mainFrame, {Size = UDim2.new(0, 600, 0, 400)})
        minimizeButton.Text = "−"
        minimized = false
    else
        CreateTween(mainFrame, {Size = UDim2.new(0, 600, 0, 50)})
        minimizeButton.Text = "+"
        minimized = true
    end
end)
...
-- Animate in
CreateTween(notification, {Position = UDim2.new(1, -320, 0, 20)}, 0.5, Enum.EasingStyle.Back)

-- Auto close
task.wait(config.Duration or 3)
CreateTween(notification, {Position = UDim2.new(1, 320, 0, 20)}, 0.5)
task.wait(0.5)
notifGui:Destroy()
```
